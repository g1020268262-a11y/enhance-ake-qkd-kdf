...
记 `Π−{x@c}` 为只在明确列出的角色、KDF 调用、context 位置删除字段 `x` 的出现位置；不删除字段本身，也不改变编码可解析性、source 顺序、label 或输出长度。两侧采用同一投影。保留 Figure 3 的消息/顺序、所有 MAC 密钥与完整输入、原有 KEM 调用及显式失败检查、`GetKey/GetKeyById` 和一次性/失败语义、`σ_KEM=k*`、`σ_QKD=k_qkd`、`k_h^1∥k_h^2` 的分割及 `τ_3` 的方向；图未明示的公钥/密文校验不擅自补入。特别是 `k*` 在 secret 输入和 context 各出现一次，若只删后者，前者仍在。初轮单字段、单位置；联合删减须另证前提在每次投影后仍成立。更改报文、MAC、QKD 接口、secret、确认或分割的方案另立版本，不借用本合同。
...
M2/M3 更新：原文形式结果的分类只表示“论文给出定理/证明”，不表示本项目已独立验证。[novelty-gap.md](novelty-gap.md) 的 B1–B7 记录了 source/context、归约余项及模型衔接问题；[security-model.md](security-model.md) 区分 H 原文和 R-CK 参考语义。尤其 R-CK 在 Test 时要求未过期，再允许后续 Expire/Corrupt；不能将上文 H 的非正式历史 Test 口径说成已核实的 CK01 规则。接受事件仍是草案，不因 M3 交付而成为原文明确事实。四个首批字段均未通过准入。
